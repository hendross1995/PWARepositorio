...
class imgUpldr {
	# Variables #
	private $_exts = array("image/jpg", "image/jpeg", "image/png", "image/gif", "image/ico", "image/bmp", "image/tif"); // Tipos de archivos soportados
	...
	private $_dest = "assets/personajesgeneradoresfotos/";
	private $_img;
	private $_ext;
	private $_r = "";
	# Métodos mágicos #
	...
	public function init($img) {
		...
		if ($this->_img['size'] <= $this->_size) {
			...
			return json_encode(array("estado"=>TRUE,"name"=>$this->_img['tmp_name'],"path"=>$this->_name,"error"=>$this->_r));
		//return $this->_r;
	}
	public function asignaNombre() {
		...
		$this->_name = sha1(date("Ymdhis")).".".$this->_ext;
	}
